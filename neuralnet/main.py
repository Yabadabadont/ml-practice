...
class MLP:
    def __init__(self, in_size, out_size, hidden_size, hidden_n, lr):
        self.in_size, self.out_size, self.hidden_size, self.hidden_n, self.lr = \
            in_size, out_size, hidden_size, hidden_n, lr

        self.layers = [Layer(in_size, hidden_size, tanh)] + \
            [Layer(hidden_size, hidden_size, tanh) for i in range(len(self.hidden_n))] + \
            [Layers(hidden_size, out_size, tanh)]
...
class Layer:
    def __init__(self, in_size, out_size, activation):
        self.in_size, self.out_size, self.activation = \
            in_size, out_size, activation

        self.weights = np.random.uniform(-0.1, 0.1, (in_size, out_size))
        self.bias = np.full(1, (in_size, out_size))
